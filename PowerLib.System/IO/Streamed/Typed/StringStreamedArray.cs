...
{
  public class StringStreamedArray : StoredFlaggedStreamedArray<String, Boolean>
  {
    #region Constructors
    ...
    public StringStreamedArray(Stream stream, Equality<String> equality, bool leaveOpen, bool boostAccess)
      : this(stream, new StringArrayParameter().With(h => { stream.Position = 0; h.Read(stream); }), equality, leaveOpen, boostAccess)
    {
    }

    protected StringStreamedArray(Stream stream, StringArrayParameter header, Equality<String> equality, bool leaveOpen, bool boostAccess)
