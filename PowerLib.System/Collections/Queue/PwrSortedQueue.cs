...
{
	public class PwrSortedQueue<T> : IQueue<T>, ICollection<T>, IEnumerable<T>, IEnumerable, ICapacitySupport, IStampSupport
	{
    ...
    private Comparison<T> _sortComparison;
		...
		public PwrSortedQueue(Comparison<T> sortComparison)
			: this(new PwrList<T>(), new InverseComparer<T>(sortComparison).Compare)
		{
		}

		public PwrSortedQueue(int capacity, Comparison<T> sortComparison)
			: this(new PwrList<T>(capacity), new InverseComparer<T>(sortComparison).Compare)
		{
		}

		public PwrSortedQueue(IEnumerable<T> coll, Comparison<T> sortComparison)
    ...
    public PwrSortedQueue(IComparer<T> sortComparer)
      : this(new PwrList<T>(), new InverseComparer<T>(sortComparer ?? Comparer<T>.Default).Compare)
    {
    }

    public PwrSortedQueue(int capacity, IComparer<T> sortComparer)
      : this(new PwrList<T>(capacity), new InverseComparer<T>(sortComparer ?? Comparer<T>.Default).Compare)
    {
    }

    public PwrSortedQueue(IEnumerable<T> coll, IComparer<T> sortComparer)
      : this((coll != null ? coll.PeekCount() : -1).WithCase(c => c >= 0, c => new PwrList<T>(c), c => new PwrList<T>()),
    ...
    protected PwrSortedQueue(IList<T> innerList, Comparison<T> sortComparison)
		{
      if (innerList == null)
